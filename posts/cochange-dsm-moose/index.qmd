...
aliases: 
  - ../../2019/08/07/CoChange-DSM-Moose.html
date: 2019-08-07
...
In a previous post, I explained how to do a [`DSM from a Java project in Moose`](../dsm-moose-java/index.qmd). In this post, I will extend the idea to a DSM-based visualization for co-change, again with a Java project using Moose.
...
A Design Structure Matrix (DSM) can be used to visualize co-change in a project, e.g., see @Geipel2012. That is, a cell *x*,*y* in the matrix contains the number of times the file *x* and *y* were changed in the same commit.
...
![Roassal CoChangeDSM visualization](RTCochangeDSM.gif)

The `GMCochangeMatrix` has the following properties with respect to the cells on its diagonal [@Geipel2012]:
